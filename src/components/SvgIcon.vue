...
const tick = ref(new Date().getMilliseconds());
...
const setValues = () => {
  if (props.fillPc) {
    ...
    tick.value = new Date().getMilliseconds();
  }
};
...
<template>
  <svg xmlns="http://www.w3.org/2000/svg" :height="height ? height : 24" :width="width ? width : 24">
    <linearGradient v-if="fillPc" :id="`lg-${tick}`" x1="0.5" y1="1" x2="0.5" y2="0">
      <stop offset="0%" :stop-color="color1" />
      <stop :offset="`${percent}%`" :stop-color="color1" />
      <stop :offset="`${percent}%`" :stop-color="color2" />
      <stop offset="100%" :stop-color="color2" />
    </linearGradient>
    <path v-if="fillPc" :d="path" :fill="`url(#lg-${tick})`" />
    <path v-else :d="path" />
  </svg>
</template>
